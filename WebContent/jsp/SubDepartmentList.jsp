...
<c:if test="${sublist==null }">
	<c:redirect
		url="/department/handle/handledepartment?departmentId=${departmentId }"></c:redirect>
</c:if>
		...
		<div id="page-wrapper">
			<div class="panel panel-info"
				...
				<div class="panel-heading">
					<h2>
						考勤系统<b>></b><strong>基本设置</strong><b>></b><strong>部门管理</strong><b>></b><strong>子部门管理</strong>
					</h2>
				...
				<div class="panel-body no-padding" style="display: block;">
					<form
						action="department/addsub/addsubdepartment?departmentId=${departmentId }"
						method="post" onsubmit="return Check()" id="form1" name="form1">
						<table class="table table-bordered">
							<tr>
								<td><font style="color: red">*&nbsp;</font>部门名称</td>
								<td><input type="text" name="departmentname"
									id="departmentName" /></td>
							</tr>
							<tr>
								<td><font style="color: red">*&nbsp;</font>上午上班时间</td>
								<td><input type="text" name="starttimeam" id="startTimeAM" />
									(时间格式为："12:00")</td>
							</tr>
							<tr>
								<td><font style="color: red">*&nbsp;</font>上午下班时间</td>
								<td><input type="text" name="endtimeam" id="startTimePM" />
									(时间格式为："12:00")</td>
							</tr>
							<tr>
								<td><font style="color: red">*&nbsp;</font>下午上班时间</td>
								<td><input type="text" name="starttimepm" id="endTimeAM" />
									(时间格式为："12:00")</td>
							</tr>
							<tr>
								<td><font style="color: red">*&nbsp;</font>下午下班时间</td>
								<td><input type="text" name="endtimepm" id="endTimePM" />
									(时间格式为："12:00")</td>
							</tr>
							<tr>
								<td><font style="color: red">*&nbsp;</font>是否支持大小礼拜</td>
								<td><input class="cls_rad" type="radio" name="weektype"
									checked="checked" value="1" />支持 &nbsp; <input class="cls_rad"
									type="radio" name="weektype" value="0" />不支持</td>
							</tr>
							<tr>
								<td align="center" colspan="2"><input class="btn btn-info" type="submit"
									value="添加" name="btnAdd" id="btnAdd" /></td>
							</tr>
						</table>
					</form>
				</div>
					...
					<div class="table-responsive">
						<table width="100%" border="0" cellpadding="8" cellspacing="0"
							...
							<c:forEach var="item" items="${sublist }">
								<tr>
									<td value="${item.departmentID }">${item.departmentID }</td>
									<td>${item.departmentName }</td>
									<td>${item.startTimeAM }</td>
									<td>${item.endTimeAM }</td>
									<td>${item.startTimePM }</td>
									<td>${item.endTimePM }</td>
									<td><a
										href="department/updateSubDepartment/updatesub?departmentId=${item.departmentID }">修改</a>
										| <a onclick="javascript:return confirm('确定要删除该子部门?')"
										href="department/subdelete/deletedesubpartment?departmentId=${item.departmentID }&parentID=${item.parentID}">删除</a>
									</td>
								</tr>
							</c:forEach>
						</table>
