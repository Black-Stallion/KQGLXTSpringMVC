...
<c:if test="${departmentlist==null }">
	<c:redirect url="/department/list/all"></c:redirect>
</c:if>
		...
		var deleteUser = function() {
			document.getElementById("deleteAdd").addEventListener('click',
					function() {
						...
						var id = "departmentId=1001";
						$.ajax({
							type : "POST",
							url : "/department/delete/deletedepartment",
							data : id + "&time=" + time,
							success : function(msg) {
								//alert(msg);
								var json = eval(msg);
								if (json.result == "exist") {
									show_err_msg('该部门含有子部门，请先删除子部门!');
									//window.location.href='SubDepartmentList.jsp';
			...
			<div class="panel panel-info"
				...
				<div class="panel-body no-padding" style="display: block;">
					<form id="form1" name="form1" onsubmit="return Check();"
						method="post" action="department/addDepartment/add">
						<table class="table table-bordered">
							<tbody>
								...
								<tr>
									...
									<td><input type="text" id="startTimeAM" name="startTimeAM" />（时间格式：12:00）</td>
								</tr>
								<tr class="success">
									<td align="center"><font color="red">*&nbsp;</font>上午下班时间</td>
									<td><input type="text" id="endTimeAM" name="endTimeAM" />（时间格式：12:00）</td>
								</tr>
								<tr>
									<td align="center"><font color="red">*&nbsp;</font>下午上班时间</td>
									<td><input type="text" id="startTimePM" name="startTimePM" />（时间格式：12:00）</td>
								</tr>
								<tr class="info">
									<td align="center"><font color="red">*&nbsp;</font>下午下班时间</td>
									<td><input type="text" id="endTimePM" name="endTimePM" />（时间格式：12:00）</td>
								</tr>
								<tr>
									<td align="center"><font color="red">*&nbsp;</font>是否支持大小礼拜</td>
									<td><input class="cls_rad" type="radio" checked="checked"
										name="weekType" value="1" />支持 &nbsp; <input class="cls_rad"
										type="radio" name="weekType" value="0" />不支持</td>
								</tr>

								<tr class="active">
									<td colspan="2" align="center"><input class="btn btn-info"
										type="submit" value="添加一级部门" name="btnAdd" id="btnAdd" /></td>
								</tr>
							</tbody>
						</table>
					</form>
						...
						<table width="100%" border="0" cellpadding="8" cellspacing="0"
							...
							<c:forEach var="item" items="${departmentlist }">
								<tr>
									...
									<td><a
										href="department/updateDep/updatedepartment?departmentId=${item.departmentID }">修改</a>
										| <a onclick="javascript:return confirm('确定要删除该部门?')"
										href="department/delete/deletepartment?departmentId=${item.departmentID }">删除</a>
										| <a
										href="department/handle/handledepartment?departmentId=${item.departmentID }">管理子部门</a>
									</td>
								</tr>
							</c:forEach>
						</table>
