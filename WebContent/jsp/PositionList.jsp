...
<c:if test="${positionlist==null }">
	<c:redirect url="/position/list/postionfindall"></c:redirect>
</c:if>
...
<body>
	<div id="wrapper">
		...
		<!-- /.navbar-static-side -->
		<form id="form1" name="form1" onsubmit="return CheckPosition()"
			method="post" action="position/addPosition/add">
		</nav>
		<div id="page-wrapper">
							...
							<tbody>
								...
								<tr class="active">
									<td colspan="2" align="center"><input class="btn btn-info"
										type="submit" value="添加" name="AddPosition" id="AddPosition" /></td>
								</tr>
							</tbody>
						</table>
					</form>
					...
					<div class="table-responsive">
						<table width="100%" border="0" cellpadding="8" cellspacing="0"
							...
							<c:forEach var="item" items="${positionlist }">
								<tr>
									<td value="${item.positionID }">${item.positionID }</td>
									<td value="${item.positionName }">${item.positionName }</td>
									<td><a
										href="position/update/jump?positionId=${item.positionID }">修改</a>
										| <a onclick="javascript:return confirm('确实要删除该职位?')"
										href="position/deletePosition/delete?positionId=${item.positionID }">删除</a>
									</td>
								</tr>
							</c:forEach>
						</table>
