...
<c:if test="${typelist==null }">
	<c:redirect url="/attendanceType/type?param=findall"></c:redirect>
</c:if>
...
<script type="text/javascript">
	...
	function CheckType() {
		var value = document.getElementById("StatusName").value;
		if (value.length == 0) {
			alert("类别名称不能为空!");
			return false;
		}
		...
		<div id="page-wrapper">
			<div class="panel panel-info"
				...
				<div class="panel-heading">
					<h2>
						考勤系统<b>></b><strong>基本设置</strong><b>></b><strong>出勤类别管理</strong>
					</h2>
				...
				<div class="panel-body no-padding" style="display: block;">
					<form id="form1" name="form1" method="post"
						action="attendanceType/type?param=addtype"
						onsubmit="return CheckType()">
						<table class="table table-bordered">
							<tr>
								<td><font style="color: red">*&nbsp;</font> 类别名称:<input
									type="text" name="StatusName" id="StatusName" /></td>
							</tr>
							<tr>
								<td><font style="color: red">*&nbsp;</font> 类别标识:<input
									class="cls_rad" type="radio" checked="checked" name="weekType"
									value="0" />非请假 &nbsp; <input class="cls_rad" type="radio"
									name="weekType" value="1" />请假</td>
							</tr>
							<tr>
								<td colspan="2" align="center"><input class="btn btn-info"
									type="submit" value="添加" name="btnAdd" id="btnAdd" /></td>
							</tr>
						</table>
					</form>
				</div>
						...
						<table width="100%" border="0" cellpadding="8" cellspacing="0"
							...
							<c:forEach var="item" items="${typelist }">
								<tr>
									...
									<td>${item.typeName }</td>
									<c:choose>
										<c:when test="${item.typeCategory== 1}">
											<td>请假</td>
										</c:when>
										<c:otherwise>
											<td>非请假</td>
									...
									<td><a onclick="javascript:return confirm('确定要删除该考勤类型?')"
										href="attendanceType/type?param=deletetype&typeid=${item.typeId }">删除</a></td>
								</tr>
							</c:forEach>
						</table>
					</div>
