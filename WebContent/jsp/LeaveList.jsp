...
%>
<c:if test="${list==null }">
	<c:redirect url="/notes/leave?param=queryAll"></c:redirect>
</c:if>
		...
		<div id="page-wrapper">
			<div class="panel panel-info"
				...
				<div class="panel-heading">
					<h2>
						考勤系统<b>></b><strong>单据管理</strong><b>></b><strong>请假单管理</strong>
					</h2>
				...
				<div class="panel-body no-padding" style="display: block;">
					<form id="form1" name="form1" method="post"
						action="notes/leave?param=query">
						<table class="table table-bordered">
							...
							<tr>
								<td align="left">请假类型:</td>
								<td><select id="noteType" name="noteType">
										<option value="0">全部</option>
										<c:forEach var="item" items="${list2}">
											<c:choose>
												<c:when test="${item.typeId==param.noteType}">
							...
							<input type="hidden" value="${param.operatorName }"
								...
								<input type="hidden" value="${param.overTime }"
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<span style="float:left">请假单列表</span>
					<span style="float:right">
						<a href="notes/leave?param=jumpadd" class="btn btn-info">添加</a>
					</span>
				</div>
						...
						<table width="100%" border="0" cellpadding="8" cellspacing="0"
							...
							<c:forEach var="item" items="${list }">
								<tr>
									...
									<td><a
										href="notes/leave?param=update&noteid=${item.noteID }">修改</a>
										| <a onclick="javascript:return confirm('是否删除?')"
										href="notes/leave?param=delete&noteid=${item.noteID }">删除</a></td>
								</tr>
							</c:forEach>
						</table>
					</div>
