		...
		<div id="page-wrapper">
			<div class="panel panel-info"
				...
				<div class="panel-heading">
					<h2>
						考勤系统<b>></b><strong>加班单管理</strong><b>></b><strong>修改加班单</strong>
					</h2>
				...
				<div class="panel-body no-padding" style="display: block;">
					<form id="form1" name="form1" method="post"
						action="notes/overtime?param=save">
						<table class="table table-bordered">
							<input type="hidden" id="noteid" name="noteid" value="${noteid }">
							<tr>
								<td><font color="red">*&nbsp;</font>申请人:</td>
								<td><input
									value="${notes.employeeID }(${notes.employeeName })"
									type="text" readonly="readonly" id="selapplicant"
									name="selapplicant" /> <input class="btn btn-info" type="button"
									id="b_selapplicant" name="b_selapplicant" value="选择申请人" /></td>
							</tr>
							<tr>
								<td><font color="red">*&nbsp;</font>代理人:</td>
								<td><input
									value="${notes.operatorID }(${notes.operatorName })"
									type="text" id="selagent" readonly="readonly" name="selagent" />
									<input class="btn btn-info" type="button" id="b_selagent"
									name="b_selagent" value="选择代理人" /></td>
							</tr>
							<tr>
								<td><font color="red">*&nbsp;</font>填表日期:</td>
								<td><input value="${notes.fillInTime }" type="date"
									placeholder="YYYY-MM-DD" id="filltime" name="filltime" /></td>
							</tr>
							<tr>
								<td><font color="red">*&nbsp;</font>加班类型:</td>
								<td><input class="cls_rad" type="radio" checked="checked"
									name="daoxiu" value="1" />需倒休 &nbsp; <input class="cls_rad"
									type="radio" name="daoxiu" value="0" />需付费</td>
							</tr>
							<tr>
								<td><font color="red">*&nbsp;</font>加班事由:</td>
								<td><textarea rows="3" cols="40" name="cause" id="cause">${notes.cause }</textarea>&nbsp;
								</td>
							</tr>
							<tr>
								<td><font color="red">*&nbsp;</font>加班起始时间:</td>
								<td><input value="${notes.startDate }" type="date"
									placeholder="YYYY-MM-DD" id="starttime" name="starttime" /> <select
									id="startmm" name="startmm">
										<c:choose>
											<c:when test="${notes.startTime=='上午' }">
												<option selected="selected" value="上午">上午</option>
												<option value="下午">下午</option>
											</c:when>
											<c:otherwise>
												<option value="上午">上午</option>
												<option selected="selected" value="下午">下午</option>
											</c:otherwise>
										</c:choose>
								</select>&nbsp;</td>
							</tr>
							<tr>
								<td><font color="red">*&nbsp;</font>加班结束时间:</td>
								<td><input value="${notes.endDate }" type="date"
									placeholder="YYYY-MM-DD" id="endtime" name="endtime" /> <select
									id="endmm" name="endmm">
										<c:choose>
											<c:when test="${notes.endTime=='上午' }">
												<option selected="selected" value="上午">上午</option>
												<option value="下午">下午</option>
											</c:when>
											<c:otherwise>
							...
							<tr>
								...
								<td><textarea rows="3" cols="40" id="directorSign"
										name="directorSign"> ${notes.directorSign }</textarea></td>
							</tr>
							<tr>
								<td>总裁意见:</td>
								<td><textarea rows="3" cols="40" name="administrationSign"
										id="administrationSign">${notes.administrationSign }</textarea>
								</td>
							</tr>
							<tr>
								<td><font color="red">*&nbsp;</font>审批结果</td>
								<c:choose>
									<c:when test="${notes.isVerify==1 }">
									...
									<c:otherwise>
										<td><input class="cls_rad" type="radio" id="result"
											name="result" value="1" />通过 &nbsp; <input checked="checked"
											class="cls_rad" type="radio" id="result" name="result"
											value="0" />未通过</td>
									</c:otherwise>
								</c:choose>
